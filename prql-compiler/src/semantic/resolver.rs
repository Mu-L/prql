        ...
        for mut stmt in stmts {
            ...
            let kind = match stmt.kind {
                ...
                StmtKind::VarDef(var_def) => {
                    ...
                    let assumed_ty = validate_type(&var_def.value, &expeceted, || {
                        Some(format!("let {}", var_def.name))
                    })?;
                    var_def.value.ty = Some(assumed_ty);

                    self.decls.declare_var(var_def, stmt.id);
                    continue;
                }
